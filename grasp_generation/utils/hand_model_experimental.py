...
class ExperimentalHandModel:
    def __init__(self, urdf_path, contact_points_path, n_surface_points=0, device='cpu'):
        ...
        for link in self.robot.links:
            ...
            # load visual mesh
            visual = link.visual
            # print(visual.geometry.filename, os.path.dirname(os.path.dirname(urdf_path)))
            filename = os.path.join(os.path.dirname(os.path.dirname(urdf_path)), "allegro_hand_description/" + visual.geometry.filename[:]) #10: before bc they had the package stuff
            # print(filename)
